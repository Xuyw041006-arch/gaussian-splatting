...
42.62%，不是新训练的提升。提交 `0922450` 已在 Colab 通过 195 项测试；完整
...
|---|---:|---:|---:|---:|---:|
| 背景 | 0.30 | 0.12 | 1.25 | 1.25 | 1 |
| 普通物品 | 1.20 | 1.25 | 0.72 | 0.70 | 2 |
| 重要物品 | 5.00 | 5.00 | 0.35 | 0.25 | 3 |

- **跨视图一致性**：将训练视角 SAM 区域的 CLIP 描述符聚成 96 个原型，除
  预处理聚合外，联合训练还把各视角渲染特征显式拉向同一个场景原型。
  Ramen 基准用 `--fit_exclude_list` 排除验证图的原型、PCA 和编码边界拟合；
  旧预处理文件必须重建后才能满足这一协议。
- **SAGA-inspired 多粒度**：SAM 区域按画面面积分为 coarse（≥25%）、
  middle（5%–25%）和 fine（<5%），训练时轮换监督并学习尺度通道门控。
- **细长物体与边界保护**：以 SAM 区域紧致度、长宽比和形态边界生成连续
  detail 权重；边界扩展 3 像素，边界与细长区域同时提升 RGB/语义权重及
  Gaussian split 优先级。3D 邻域平滑增加语义 affinity，避免跨边界抹平。
- **动态资源分配**：重要高斯更容易分裂、更难被删除，背景/普通/重要分别使用
  一/二/三阶 SH。随仓库提供的 CUDA 前向与反向只实现到三阶；旧版配置中的
  四、五阶参数并未带来对应的高阶拟合。分档参数屏蔽也不等于分桶光栅化，
  实际速度与显存收益需要 GPU 测量。

高斯重要性投影现与 CUDA `ndc2Pix` 的像素中心和 Y 方向一致；旧权重中的错误
重要性分配不会因更新代码自动修复。语义验证固定中粒度、覆盖全部通道，
不再用随迭代变化的通道子集比较损失。
...
pilot 中重要监督像素占 22.89%，内部重要性标签下的高斯占 44.58%。这些统计
不能单独证明真实物体上的资源分配正确，尤其旧版存在投影方向问题。表内只说明
这一次短训练的重要类别 IoU 较高；全局画质和边界质量仍落后，不能外推为稳定提升。
...
基准脚本默认 RGB 上限 15,000 轮、固定迭代模式的顺序语义上限 5,000 轮；
默认联合语义从第 1,000 轮开启，在第 11,250 轮结束 densification。
v4 恢复任务显式使用 1,500 轮 RGB warm-up；通用均衡产品预设的分裂截止为
12,000 轮，两种入口参数有所不同，实际命令应随结果保存。

脚本从 131 张源训练图中均匀留出 12 张验证图，其余 119 张用于优化；到达验证
起始轮次后每 1,000 轮评估。保存最佳权重与早停容差已经分开：任意更高验证
PSNR 都保存 checkpoint，`0.02 dB` 只控制早停耐心计数。连续 4 次没有显著改善
可早停；最佳权重可能以 `iteration_15000` 导出，真实来源须看
`validation_summary.json` 与评估的 `weight_selection`。

等时间是请求的比较协议，不是只传参数就成立的结果。联合、顺序 RGB、顺序语义
三个阶段都需要完整累计时长与完成标志；恢复前的时间缺失时必须标为不能认证。
`comparison.json` 的 `protocol.equal_wall_clock` 现由实测认证，不能沿用旧版
仅表示请求的布尔值。3 个标注视角的 `test_psnr` 与训练日志中全部 4 个测试视角
的 PSNR 也不能直接相减。

使用独立目录保存修正前的 v3 与新训练的 `ramen_corrected_v4_15k`，并核对当前
Notebook 的实际持久化路径。`--resume` 只对仍存在且完整的 checkpoint 生效；
本地 `/content` 会随 Colab 运行时回收。清理云盘前保留最近可恢复 checkpoint、
最佳权重、最终 PLY 与语义文件、评估和计时记录。
