...
class Scene:
    ...
    def __init__(self, args : ModelParams, gaussians : GaussianModel, load_iteration=None, shuffle=True, resolution_scales=[1.0]):
        ...
        if len(selected_train_cameras) != len(scene_info.train_cameras):
            if len(selected_train_cameras) < 2:
                raise ValueError("Sparse-view selection left fewer than two cameras")
            print(
                f"Sparse-view mode: using {len(selected_train_cameras)} of "
                f"{len(scene_info.train_cameras)} training views"
            )
            scene_info = scene_info._replace(
                train_cameras=selected_train_cameras,
                nerf_normalization=getNerfppNorm(selected_train_cameras),
            )
